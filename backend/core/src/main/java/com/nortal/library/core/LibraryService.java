...
public class LibraryService {
  ...
  public Result borrowBook(String bookId, String memberId) {
    ...
    if (entity.getLoanedTo() == null) { // 1. Just if book is free, other member can borrow
      if (!entity.getReservationQueue().isEmpty()) { // 2. No line-jumping via direct borrow
        if (!entity.getReservationQueue().get(0).equals(memberId)) {
          return Result.failure("GO_TO_QUEUE");
        }
  ...
  public ResultWithNext returnBook(String bookId, String memberId) {
    ...
    while (!entity.getReservationQueue().isEmpty()) { // 3. After reservaton limit controll
      if (canMemberBorrow(entity.getReservationQueue().get(0))) {
        nextMember = entity.getReservationQueue().remove(0);
        break;
      } else {
        entity.getReservationQueue().remove(0);
      }
    }
    if (nextMember == null) { // 1. If queue is empty - book is free, if reserved - first get it
  ...
  public Result reserveBook(String bookId, String memberId) {
    ...
    Book entity = book.get();
    if (entity.getLoanedTo() == null) { // 2. If book is free, don't reserve => borrow
      if (!canMemberBorrow(memberId)) {
        return Result.failure("BORROW_LIMIT");
      }
      entity.setLoanedTo(memberId);
    ...
    if (entity
        .getLoanedTo()
        .equals(memberId)) { // 2. Duplicate reservations by the same member are rejected
      return Result.failure("BORROWED_BY_THE_SAME_MEMBER");
    }
    if (entity
        .getReservationQueue()
        .contains(memberId)) { // 2. Don't reserve the same person in queue
      return Result.failure("RESERVED_BY_THE_SAME_MEMBER");
    }
